...
def oslo_stations():  
    ...
    return oslo_stations_info


def get_all_obs (from_date, to_date):
    all_obs_list = []
    oslo_stations_info = oslo_stations()
    oslo_station_names = []
    all_obs_list = []
...
def insert_p_t_fact(to_date, from_date):
    ...
    df = df.pivot_table(index = ['sk_date','sk_air_quality'], columns = ['component'], 
                        values = 'value').fillna(np.nan).reset_index()
    #return df
    conn = get_connection()
    with conn as connection:
        cursor = connection.cursor()
...
#insert_p_t_fact('2021-06-30', '2021-09-13')
...
def update_facts_table():
    ...
    with conn as connection:
        cursor = connection.cursor()
        cursor.execute('select max(sk_date) from facts_air_quality')
        last_date_tuple = cursor.fetchone()
    ...
    insert_p_t_fact(from_date_plus, to_date)
